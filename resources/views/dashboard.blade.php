    ...
    <div class="topbar-right">
        ...
        <section class="content">
            ...
            <div class="dashboard-grid">
                    ...
                    <table class="snapshot-table">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>{{ ($row->clock_in_time && $row->clock_in_time !== '--:--') ? \Carbon\Carbon::parse($row->clock_in_time)->format('h:i A') : '--:--' }}</td>
                                <td>
                ...
                <div class="side-widgets">
                    <div class="panel" style="margin-bottom: 20px; max-height: 400px; overflow-y: auto;">
                        <div class="panel-header" style="margin-bottom: 15px;">
                            <h4 style="font-size: 14px; margin: 0;">📍 Today's External Tasks</h4>
                            <a href="{{ route('tasks.assigned') }}" style="font-size: 11px; color: var(--doremi-red); text-decoration: none; font-weight: 600;">View All →</a>
                        </div>
                        
                        @if(isset($todays_tasks) && count($todays_tasks) > 0)
                            <div style="display: flex; flex-direction: column; gap: 10px;">
                                @foreach($todays_tasks as $task)
                                    <div style="background: #f9fafb; border: 1px solid #e5e7eb; border-radius: 8px; padding: 12px;">
                                        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 5px;">
                                            <span style="font-weight: 600; font-size: 13px; color: #111827;">{{ $task->staff_name }}</span>
                                            <span style="font-size: 11px; color: #6b7280; background: #e5e7eb; padding: 2px 6px; border-radius: 4px; font-weight: 600;">{{ ($task->time && $task->time !== '--:--') ? \Carbon\Carbon::parse($task->time)->format('h:i A') : '--:--' }}</span>
                                        </div>
                                        <div style="font-size: 12px; color: #4b5563; display: flex; align-items: center; gap: 5px;">
                        ...
                        @else
                            <div style="text-align: center; padding: 20px; color: #9ca3af; font-size: 12px; background: #f9fafb; border-radius: 8px; border: 1px dashed #d1d5db;">
                                No external tasks assigned today.
                            </div>
                        @endif
                    </div>
                </div>
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function() {
                const projectId = '{{ env("FIRESTORE_PROJECT_ID", "doremi-admin2") }}';
