...
class AnalyticsController extends Controller
{
    ...
    public function index(Request $request)
    {
        try {
            ...
            $attDocs   = Http::get("{$this->baseUrl()}/attendances")->json()['documents'] ?? [];
            $leaveDocs = Http::get("{$this->baseUrl()}/leaves")->json()['documents'] ?? [];
            $otDocs    = Http::get("{$this->baseUrl()}/overtime")->json()['documents'] ?? [];
            ...
            foreach ($leaveDocs as $doc) {
                ...
                if ($type === 'AL' || $type === 'Annual')        $annual    += $days;
                elseif ($type === 'MC' || $type === 'Medical')   $mc        += $days;
                elseif ($type === 'EL' || $type === 'Emergency') $emergency += $days;
        ...
        foreach($leaveDocs as $doc) {
            ...
            if (($f['status']['stringValue'] ?? '') === 'Approved') {
                ...
                if ($carbonDate->year == $yearInput && ($isYearly || $carbonDate->month == $monthInput)) {
                    ...
                    $type = $f['leave_type']['stringValue'] ?? $f['leaveType']['stringValue'] ?? '';
                    if ($type === 'AL' || $type === 'Annual') $annual += $d;
                    elseif ($type === 'MC' || $type === 'Medical') $mc += $d;
                    else $emergency += $d;
                    $totalDaysMonth += $d;
                }
            }
