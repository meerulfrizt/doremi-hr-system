...
{
    private string $projectId = env('FIREBASE_PROJECT_ID', 'doremi-admin');
    ...
    {
        try {
            $docs    = Http::get("https://firestore.googleapis.com/v1/projects/{$this->projectId}/databases/(default)/documents/overtime")->json()['documents'] ?? [];
            ...
            $attRes = Http::get("https://firestore.googleapis.com/v1/projects/{$this->projectId}/databases/(default)/documents/attendances")->json()['documents'] ?? [];
            $attendanceMap = [];
...
4. PRODUCTIVE:
   ...
   - Output/deliverable can be cross-referenced";
    ...
    {
        try {
            $docs = Http::get("https://firestore.googleapis.com/v1/projects/{$this->projectId}/databases/(default)/documents/leaves")->json()['documents'] ?? [];
            ...
            foreach ($docs as $doc) {
                ...
                if (trim($uid) !== trim($userId)) continue;
                $dateRaw = $f['startDate']['timestampValue'] ?? ($f['startDate']['stringValue'] ?? '');
                if (empty($dateRaw)) continue;
                try { $date = \Carbon\Carbon::parse($dateRaw); } catch (\Exception $e) { continue; }
                $type  = $f['leaveType']['stringValue'] ?? 'Other';
                $days  = (int)($f['totalDays']['integerValue'] ?? 1);
                $records[] = ['date' => $date->format('Y-m-d'), 'type' => $type];
                $totalDays += $days;
                $dayCount[$date->dayOfWeek]++;
                $up = strtoupper($type);
                if (str_contains($up, 'ANNUAL') || str_contains($up, ' AL')) $typeCount['AL']++;
                elseif (str_contains($up, 'EMERGENCY') || str_contains($up, ' EL')) $typeCount['EL']++;
    ...
    {
        try {
            $uf           = Http::get("https://firestore.googleapis.com/v1/projects/{$this->projectId}/databases/(default)/documents/users/{$userId}")->json()['fields'] ?? [];
            $flexiBal     = (float)($uf['ot_balance']['integerValue'] ?? ($uf['ot_balance']['doubleValue'] ?? 0));
            $otDocs       = Http::get("https://firestore.googleapis.com/v1/projects/{$this->projectId}/databases/(default)/documents/overtime")->json()['documents'] ?? [];
    ...
    public function kpiReview(string $userId)
    {
        try {
            ...
            // Fetch user profile
            $uf = Http::get("https://firestore.googleapis.com/v1/projects/{$projectId}/databases/(default)/documents/users/{$userId}")->json()['fields'] ?? [];
            ...
            $attDocs  = Http::get("https://firestore.googleapis.com/v1/projects/{$projectId}/databases/(default)/documents/attendances")->json()['documents'] ?? [];
            ...
            // OT last 90 days
            $otDocs = Http::get("https://firestore.googleapis.com/v1/projects/{$projectId}/databases/(default)/documents/overtime")->json()['documents'] ?? [];
            ...
            $leaveDocs = Http::get("https://firestore.googleapis.com/v1/projects/{$projectId}/databases/(default)/documents/leaves")->json()['documents'] ?? [];
