...
class EmployeeController extends Controller
{
    ...
    public function index(Request $request)
    {
        try {
            ...
            // Kira pending notifications (leaves + OT + flexi)
            $pendingTotal = 0;
            try {
                $collections = ['leaves', 'overtime', 'flexi'];
                foreach ($collections as $col) {
                    $docs = Http::get("https://firestore.googleapis.com/v1/projects/{$this->projectId}/databases/(default)/documents/{$col}")->json()['documents'] ?? [];
                    foreach ($docs as $doc) {
                        if (strtolower(($doc['fields']['status']['stringValue'] ?? '')) === 'pending') {
                            $pendingTotal++;
                        }
                    }
                }
    ...
    {
        $projectId = env('FIREBASE_PROJECT_ID', $this->projectId);
    ...
    {
        try {
            $url = "https://firestore.googleapis.com/v1/projects/{$this->projectId}/databases/(default)/documents/users/{$id}";
            $response = Http::get($url);
            $doc = $response->json();
    ...
    public function show($id)
    {
        try {
            // Tarik Info Staf
            $userUrl = "https://firestore.googleapis.com/v1/projects/{$this->projectId}/databases/(default)/documents/users/{$id}";
            ...
            $attUrl = "https://firestore.googleapis.com/v1/projects/{$this->projectId}/databases/(default)/documents/attendances";
