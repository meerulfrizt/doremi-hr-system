...
class SeedFirestoreDummyData extends Command
{
    ...
    private $projectId = 'doremi-admin2';
    ...
    public function handle()
    {
        ...
        $usersResponse = Http::withoutVerifying()->retry(3, 1000)->withOptions(['connect_timeout' => 30])->timeout(30)->get("https://firestore.googleapis.com/v1/projects/{$this->projectId}/databases/(default)/documents/users");
        if ($usersResponse->successful() && isset($usersResponse->json()['documents'])) {
            foreach ($usersResponse->json()['documents'] as $doc) {
                ...
                if (str_ends_with($email, '@doremi.com')) {
                    Http::withoutVerifying()->retry(3, 1000)->withOptions(['connect_timeout' => 30])->timeout(30)->delete("https://firestore.googleapis.com/v1/projects/{$this->projectId}/databases/(default)/documents/users/" . basename($doc['name']));
                }
            }
        }
        
        // Clear existing OT requests
        $otResponse = Http::withoutVerifying()->retry(3, 1000)->withOptions(['connect_timeout' => 30])->timeout(30)->get("https://firestore.googleapis.com/v1/projects/{$this->projectId}/databases/(default)/documents/overtime");
        if ($otResponse->successful() && isset($otResponse->json()['documents'])) {
            foreach ($otResponse->json()['documents'] as $doc) {
                Http::withoutVerifying()->retry(3, 1000)->withOptions(['connect_timeout' => 30])->timeout(30)->delete("https://firestore.googleapis.com/v1/projects/{$this->projectId}/databases/(default)/documents/overtime/" . basename($doc['name']));
            }
        }

        // Clear existing Flexi requests
        $flexiResponse = Http::withoutVerifying()->retry(3, 1000)->withOptions(['connect_timeout' => 30])->timeout(30)->get("https://firestore.googleapis.com/v1/projects/{$this->projectId}/databases/(default)/documents/flexi");
        if ($flexiResponse->successful() && isset($flexiResponse->json()['documents'])) {
            foreach ($flexiResponse->json()['documents'] as $doc) {
                Http::withoutVerifying()->retry(3, 1000)->withOptions(['connect_timeout' => 30])->timeout(30)->delete("https://firestore.googleapis.com/v1/projects/{$this->projectId}/databases/(default)/documents/flexi/" . basename($doc['name']));
            }
        }
        ...
        foreach ($dummyUsers as $index => $user) {
            ...
            $userResponse = Http::withoutVerifying()->retry(3, 1000)->withOptions(['connect_timeout' => 30])->timeout(30)->post("https://firestore.googleapis.com/v1/projects/{$this->projectId}/databases/(default)/documents/users", $userPayload);
    ...
    private function createFlexiRequests($uid, $staffName, $timestamp)
    {
        ...
        $flexResponse = Http::withoutVerifying()->retry(3, 1000)->withOptions(['connect_timeout' => 30])->timeout(30)->post("https://firestore.googleapis.com/v1/projects/{$this->projectId}/databases/(default)/documents/flexi", $flexiPayload);
    ...
    private function createOvertimeRequests($uid, $staffName, $timestamp)
    {
        ...
        foreach ($otRequests as $index => $req) {
            ...
            $otResponse = Http::withoutVerifying()->retry(3, 1000)->withOptions(['connect_timeout' => 30])->timeout(30)->post("https://firestore.googleapis.com/v1/projects/{$this->projectId}/databases/(default)/documents/overtime", $otPayload);
